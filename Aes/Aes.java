...
public class Aes {
    ...
    public String addRoundKey(String input1,String input2){
        String output;
    ...
    public String shiftRowOnRound(String input){
        String output="";
    ...
    public String mixColumn(String input){
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                B[j][i]= function.mixColumnOne(mixColumn, i, j);
            }
        }
        //B=function.convertStringToMatric(output);
        output="";
    ...
    public void expandKey(String inputKey){
        String temp;
        for(int i=0;i<4;i++){
            words[i]="";
    ...
    public void excuteRound(){
        ...
        printString(roundState[0]);
        for(int i=1;i<=10;i++){
            String outputSubByteOnRound=function.subByte(roundState[i-1]);
            System.out.printf("outputSubByteOnRound on Round %d is: ",i);
            printString(outputSubByteOnRound);
            String outputShiftRowOnRound=shiftRowOnRound(outputSubByteOnRound);
            ...
            printString(key[i]);
            roundState[i]=addRoundKey(outMixColumn, key[i]);
